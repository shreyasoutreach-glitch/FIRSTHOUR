...
const ACTOR_COLOR: Record<string, string> = {
  SYSTEM: "text-text_primary/60 border-forest/20",
...
export default function Audit() {
  ...
  return (
    <div className="max-w-canvas mx-auto px-6 sm:px-10 py-16">
      ...
      {!events && !error && <p className="text-text_primary/40 text-[14px]">Loading audit trail&hellip;</p>}

      {events && events.length === 0 && (
        <p className="text-text_primary/45 text-[14px]">No audit events recorded for this case yet.</p>
      )}
      ...
      {events && events.length > 0 && (
        <div className="max-w-[900px] overflow-x-auto">
          <table className="w-full text-[13px] border-collapse">
            <thead>
              <tr className="text-left text-text_primary/45 text-[11px] uppercase tracking-wide border-b border-gold/25">
            ...
            <tbody>
              {events.map((e) => (
                <tr key={e.id} className="border-b border-gold/10 align-top">
                  <td className="py-3 pr-4 text-text_primary/55 whitespace-nowrap">{formatDateTime(e.created_at)}</td>
                  <td className="py-3 pr-4">
                    <span className={`text-[10.5px] uppercase tracking-wide border rounded-full px-2 py-0.5 ${ACTOR_COLOR[e.actor] || "text-text_primary/50 border-forest/20"}`}>
                      {e.actor}
                    </span>
                  </td>
                  <td className="py-3 pr-4 font-ui">{e.event_type}</td>
                  <td className="py-3 pr-4 text-text_primary/75 max-w-[280px]">{e.summary}</td>
                  <td className="py-3 text-text_primary/40 text-[12px] max-w-[220px]">
